...
import { UserDefinedMessageSubscriptionContextImpl } from "twilio/lib/rest/api/v2010/account/call/userDefinedMessageSubscription.js";
   ...
   if (! validatePhoneNumber(phone)) {
    ...
    try {
        ...
        if (user.verificationCode !== Number(otp)) {
          return next( new ErrorHandler("Invalid OPT.", 400));
        }
        ...
        if (currentTime > verificationCodeExpire) {
          return next(new ErrorHandler("OPT expired.", 400));
        }
...
export const login = catchAsyncError(async (req, res, next) => {
  ...
  try {
    ...
    if (!isPasswordMatched) {
      console.log("Password mismatch for user:", email); // Log if password doesn't match
      return next(new ErrorHandler("Invalid email or password.", 400));
    }
    ...
    return next(new ErrorHandler("Login failed. Please try again later.", 500)); // Generic error message for security
  }
});
...
export const forgotPassword = catchAsyncError( async (req, res, next) => {
  ...
  const message = `Your Reset Passport Token is : \n\n ${resetPasswordUrl} n\n\ If you have not requested this email then please ignore it.`
...
export const resetPassword = catchAsyncError(async(req, res, next) => {
  ...
  const resetPasswordToken = crypto.createdAt("sha256".update(token).digest("hex"));
  const user = await User.findOne({
    resetPasswordToken,
  ...
  if (!user) {
   ...
   if (req.body.password !== req.body.confirmPassword) {
    return next (
      new ErrorHandler(
        "Password $ confirm password do not match.", 400 )
   );
  }
